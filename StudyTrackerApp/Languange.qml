...
Item {
    ...
    readonly property string totalFokusStat:     t("Total waktu fokus per hari: {menit} menit ({jam} jam {sisa} menit)",
                                                   "Total focus time per day: {menit} min ({jam} hr {sisa} min)",
                                                   "每日总专注时间：{menit} 分钟（{jam} 小时 {sisa} 分钟）",
                                                   "1日の総集中時間：{menit} 分（{jam} 時間 {sisa} 分）",
                                                   "하루 총 집중 시간: {menit}분 ({jam}시간 {sisa}분)",
