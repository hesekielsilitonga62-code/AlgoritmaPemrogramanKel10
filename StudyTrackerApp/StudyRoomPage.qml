...
Rectangle {
    ...
    ColumnLayout {
        ...
        // DAFTAR TUGAS — collapsible, scroll internal, tidak ganggu grid teman
        Rectangle {
            id: taskPanel
            Layout.fillWidth: true
            // Tinggi SELALU fixed: header 36px + isi max 120px (scroll)
            // Berapapun tugas ditambah, ukuran panel tidak melewati batas ini
            Layout.preferredHeight: taskPanelExpanded
                                    ? 36 + 1 + Math.min(taskInnerList.contentHeight + 8, 120)
                                    : 36
            ...
            property bool taskPanelExpanded: false
            ...
            // ── Header bar — selalu tampil, klik untuk buka/tutup ────────
            Rectangle {
                ...
                RowLayout {
                    ...
                    Rectangle {
                        width: badgeNum.implicitWidth + 10; height: 18; radius: 9
                        color: Qt.rgba(window.accentColor.r, window.accentColor.g, window.accentColor.b, 0.18)
                        border.color: window.accentColor; border.width: 1
                        visible: taskPanel.pendingCount > 0
                        Text {
                            id: badgeNum
                            anchors.centerIn: parent
                            text: taskPanel.pendingCount
                            color: window.accentColor; font.pixelSize: 9; font.bold: true
                        }
                    }

                    Text {
                        text: taskPanel.pendingCount === 0
                              ? lang.belumAdaTugas
                              : (taskPanel.taskPanelExpanded ? "▲" : "▼")
                ...
                MouseArea {
                    ...
                    onClicked: if (taskPanel.pendingCount > 0)
                                   taskPanel.taskPanelExpanded = !taskPanel.taskPanelExpanded
                }
            }

            // Divider
            Rectangle {
                y: 36; width: parent.width; height: 1
                color: window.borderColor
                opacity: taskPanel.taskPanelExpanded ? 0.6 : 0
                Behavior on opacity { NumberAnimation { duration: 180 } }
            }

            // ── Isi list scrollable — max tinggi 120px ────────────────────
            ListView {
                id: taskInnerList
                x: 0; y: 37
                width: parent.width - (contentHeight > 120 ? 14 : 0)
                ...
                ScrollBar.vertical: ScrollBar {
                    policy: taskInnerList.contentHeight > 120
                            ? ScrollBar.AlwaysOn : ScrollBar.AlwaysOff
                }

                delegate: Rectangle {
                    width: taskInnerList.width
                    height: model.isDone ? 0 : 30
                    visible: !model.isDone
                    color: window.bgCard; radius: 7
                    clip: true
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            ...
                            color: window.accentColor; opacity: 0.85
                        }
                        Text {
                            text: model.title
                        ...
                        Text {
                            text: model.deadline
                            color: window.accentColor; font.pixelSize: 10
                        }
                    }
                }
            }
