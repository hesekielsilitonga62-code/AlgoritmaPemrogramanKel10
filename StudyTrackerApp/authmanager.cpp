...
bool AuthManager::resetPassword(QString username, QString newPassword)
{
    if (!checkUserExists(username)) {
        qDebug() << "[AuthManager] Reset gagal - user tidak ditemukan:" << username;
        return false;
    }
    writePassword(username, newPassword);
    qDebug() << "[AuthManager] Password untuk" << username << "berhasil diperbarui.";
    return true;
}
